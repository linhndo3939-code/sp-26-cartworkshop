...
- Hooks: `src/hooks/` (e.g., useCartContext.ts is here)
- Contexts: `src/contexts/` (CartContext.tsx)
- Reducers: `src/reducers/` (cartReducer.ts)
- Types: `src/types/` (product.ts, cart.ts)
- Styling: Using CSS Modules (`*.module.css`)
## Code Style
...
## Development Notes
- **Strict Typing:** Always explicitly type items in array methods (e.g., `(item: CartItem) => ...`) to avoid implicit 'any' errors in the reducer.
- **Hook Location:** The `useCartContext` hook lives in `src/hooks/`, NOT the context file, to avoid Fast Refresh issues.
- **Dispatch Pattern:** When calling `dispatch`, the payload must follow the structure: `{ payload: { productId, ... } }`.